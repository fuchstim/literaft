...
// Wal-index layout constants, cross-checked against SQLite's wal.c and
// this build's actual on-disk output (see README.md). All wal-index
...
// header is the in-memory form of one WalIndexHdr copy. salt is carried as
// an opaque 8-byte blob, never interpreted as a number: SQLite itself only
// ever memcpy's it between the WAL header and frame headers, so its byte
...
func (h header) encode() [hdrCopySize]byte {
...
// readHeader reads the wal-index header using the same tear-safe protocol
...
func readHeader(region0 []byte) (header, bool) {
	for _, b := range [][]byte{
...
// writeHeader publishes h into region0 using SQLite's tear-safe two-copy
...
// copy 0 is replaced).
func writeHeader(region0 []byte, h header) {
	b := h.encode()
...
// proceeds. Mirrors ncruces' own shmBarrier: a real atomic operation used
// purely for its hardware/compiler fence side effect, not its value.
func barrier() {
	var b atomic.Bool
	b.Swap(true)
}
...
// insertFrame records, within the wal-index page for frame, that WAL
...
func insertFrame(region []byte, frame, pgno uint32) {
