...
// golang.org/x/sys/unix; these raw values are pinned from Darwin's own
// fcntl.h.
const (
	_F_OFD_SETLK  = 90
	_F_OFD_SETLKW = 91
...
// classic per-process fcntl locks, are required because multiple *os.File
// opens of the same path within this one process must hold independent
// lock instances, not silently merge or release each other's on close.
