...
freshly generated by `Gate.propose` for every call). Its job is narrower than
it looks: it's how `fsm.FSM.Apply` recognizes "this is the one proposal I'm
still in the middle of" so it doesn't re-materialize via
...
`Gate.propose` marks the ID in `fsm.FSM.skipEntries` immediately before its
own `hraft.Apply` call and clears it (deferred) immediately after that call
returns — see ADR-011 for why that transience, not a permanent per-entry
property, is what makes this safe. There is no client-request-ID field for
...
3. On the commit frame, `internal/vfs.File` calls `Gate.Propose(txn)`
   (`internal/raft/gate.Gate`, the concrete implementation of the `vfs.Gate`
   interface) with the whole captured batch as a `*raftproto.Transaction`, and
   **blocks**. `Gate.propose` wraps it in an `internal/raft/proto.Entry` with
   a fresh per-proposal `Header.Id` and hands that to `hraft.Raft.Apply` (see
   `§RAFT log entry format` above). RAFT needs no SQLite lock, so readers
   keep going; only other *writers* wait (they're behind the write lock
   anyway).
...
4. **RAFT commits (quorum):** `Gate.Propose` returns `nil`; `internal/vfs.File`
   ...
   us the exact tear-safe protocol for free. The proposing node's own
   `fsm.FSM.Apply` for this same committed entry sees its `Header.Id` still
   marked in `skipEntries` (set just before this `hraft.Apply` call, cleared
   right after it returns) and skips re-materializing via `walappender` —
   everyone else applies it normally.
...
1. `fsm.FSM.Apply` decodes the committed `internal/raft/proto.Entry` and
   ...
   (this node proposed it and is still inside the one `Gate.propose` call
   that published it), **return without touching the WAL at all** — this
   node's own SQLite write path already published it (`§write path` step 4).
   Otherwise, continue.

   The marker is transient (set/cleared around one `hraft.Apply` call, not a
   permanent property of the entry) precisely so it *doesn't* fire for a
...
- *Gaining leadership:* a node can be **apply-behind** (has every committed log
  ...
  *during* the drain, before any new self-proposal can even start (`Ready`,
  and so `Gate.propose`, stays closed until the drain finishes) — so there's
  no live per-proposal `skipEntries` marker for it to be mistaken against.
