...
> changed and why, including the self-apply skip correctness bug that
> restructuring introduced and its later fix (ADR-011).
...
deliberately thin and hraft-agnostic: it encodes a captured write transaction
...
transport/log-store/snapshot-store, builds the `fsm.FSM`, wraps the
`hraft.Raft` in a `log.NewSingleWriterLog`, and hands the `fsm.FSM` plus that
`LogAdapter` to `driver.New`. There is no longer an intermediate "node"
package doing this wiring on `driver`'s behalf (see `DECISIONS.md` ADR-013) —
`driver.New` and the direct-caller pattern *is* that wiring layer now.
...
`Header.Id` is not a node identifier — it's a per-proposal token (a UUID,
freshly generated by `Gate.proposeTransaction` for every call). Its job is
narrower than it looks: it's how `fsm.FSM.Apply` recognizes "this is the one
proposal I'm still in the middle of" so it doesn't re-materialize via
...
before its own `LogAdapter.Apply` call (which, for `log.SingleWriterLog`, is
what actually invokes `hraft.Raft.Apply`) and clears it (deferred)
immediately after that call returns — see ADR-011 for why that transience,
not a permanent per-entry property, is what makes this safe. There is no client-request-ID field for
ambiguous-commit dedup (`ADR-008`/`ADR-009` still call this out as needed
once forwarding exists) — that remains fully deferred, unlike an earlier
draft of this document which described a reqID field as already present; it
never was, in either the original or current entry format. (`Header.Id`
looks similar but serves the self-apply skip specifically, not
ambiguous-commit dedup — the two must not be conflated if a reqID field is
ever added.)
...
4. **RAFT commits (quorum):** `Gate.ProposeTransaction` returns `nil`;
   ...
   publish gives us the exact tear-safe protocol for free. The proposing
...
1. `fsm.FSM.Apply` decodes the committed `internal/raft/proto.Entry` and
   ...
   not a permanent property of the entry) precisely so it *doesn't* fire for a
...
drain) lives in `log.SingleWriterLog` now, not `internal/raft/gate.Gate` — see
`§Architecture` above. `Gate` itself has no opinion on any of it; it just
...
- You can skip/batch the local WAL fsync. A crashed node rebuilds its WAL tail
  ...
  rather than corrupting it. (See ADR-011 for the self-apply skip's own,
  separate correctness history — a distinct concern from ordinary
  crash/restart recovery, now fixed by making the skip transient.)
