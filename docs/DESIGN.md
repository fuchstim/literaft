...
> "Refactor" commit along with the rest of `docs/` and the whole test suite,
> then reconstructed afterward against the resulting architecture. Package
> names below reflect the current tree (`fsm/`, `internal/fsm/walappender/`,
> `internal/fsm/snapshotter/`, `internal/raft/gate/`, `internal/raft/proto/`,
> `internal/vfs/`, `driver/`) — see ADR history in `DECISIONS.md` for what
> changed and why, and CLAUDE.md's "Top gotchas" for the one known,
> tracked correctness bug this restructuring introduced (self-apply skip).
...
```
  ...
  one running node (cmd/literaft/main.go's run(), or driver.New for an embedder):
      hraft.Raft + fsm.FSM  ──►  driver.New  ──►  internal/raft/gate.Gate + internal/vfs.Register
                                     │
...
`hraft.FSM` directly (`Apply`/`Snapshot`/`Restore`). `driver.New(r, fsm, opts...)`
takes a real `*hraft.Raft` and `*fsm.FSM` the caller already constructed,
builds the `internal/raft/gate.Gate`, registers a process-unique gated VFS
(`internal/vfs.Register`), and returns a `database/sql`-compatible
`driver.Driver`. `cmd/literaft/main.go`'s `run()` is the reference caller —
it wires the `hraft.Raft` transport/log-store/snapshot-store, builds the
`fsm.FSM`, and hands both to `driver.New`. There is no longer an intermediate
"node" package doing this wiring on `driver`'s behalf (see `DECISIONS.md`
ADR-013) — `driver.New` and the direct-caller pattern *is* that wiring layer
now.
...
the proposing node's ID, an ordered list of `(pgno, page_image)` pairs, and
`nTruncate` (the post-commit database size in pages, which doubles as the
commit marker).

Rationale for physical redo over the page list is in `DECISIONS.md` ADR-003.
Short version: full page images under a total order are self-consistent by
...
The `NodeID` field's job is narrower than it looks: it's how `fsm.FSM.Apply`
recognizes "this is my own entry" so it doesn't re-materialize via
`walappender.AppendEntry` something the leader already published itself via
its own SQLite write path (`ADR-005`, `§follower-apply` below). **This is
also where the one known, tracked bug in the current design lives — see
`DECISIONS.md` ADR-011 before touching this field's handling.** There is no
client-request-ID field for ambiguous-commit dedup (`ADR-008`/`ADR-009`
still call this out as needed once forwarding exists) — that remains fully
deferred, unlike an earlier draft of this document which described a
reqID field as already present; it never was, in either the original or
...
3. On the commit frame, `internal/vfs.File` calls `Gate.Propose(frames, nTruncate)`
   (`internal/raft/gate.Gate`, the concrete implementation of the `vfs.Gate`
   interface) with the whole captured batch, and **blocks**. `Gate.Propose`
   embeds this node's ID in the `internal/raft/proto.Entry` it hands to
...
4. **RAFT commits (quorum):** `Gate.Propose` returns `nil`; `internal/vfs.File`
   flushes the withheld commit frame to the real `-wal`, then returns
   `SQLITE_OK` from `xWrite`. SQLite proceeds normally: optional `xSync`, then
   `walIndexAppend` + `walIndexWriteHdr` — the standard two-copy, barriered
   header publish that advances `mxFrame`. The txn is now visible, and
   correctly so. **We never touch the shm here** — SQLite's own publish gives
   us the exact tear-safe protocol for free. Every *other* node's
   `fsm.FSM.Apply` for this same committed entry sees `entry.NodeID` equal to
   its own only on the node that proposed it, so exactly that one node skips
...
5. **RAFT fails** (lost leadership / timeout / no quorum / not-leader):
   `Gate.Propose` returns an error (`*raftgate.NotLeaderError`,
   `raftgate.CatchingUpError`, or a wrapped `hraft` error); `internal/vfs.File`
   discards the buffered commit frame and returns an I/O error from `xWrite`.
   The failure occurs *before* the wal-index append, so `mxFrame` never moves
...
not VFS). Still deferred — see `DECISIONS.md` ADR-008/009's neighbors in
`ROADMAP.md`'s "Deferred" section.
...
`fsm.FSM`'s `walappender.WALAppender` drives this itself now (`runCheckpointer`):
...
dirty pages have accumulated (default 1000) — unconditionally, on every node
regardless of leader/follower role, since a passive checkpoint is harmless
(and a no-op if it can't make progress) either way. `internal/vfs` does not
intercept `wal_checkpoint` itself.
...
`internal/fsm/snapshotter.Snapshotter.Snapshot` currently does *not* force
this checkpoint itself (unlike the pre-refactor `dbBackend.Snapshot`) — it
calls SQLite's backup API directly against whatever's on disk, which is
correct either way (Backup resolves pages still only in the `-wal`), just
potentially slower under a busy concurrent writer.
...
1. `fsm.FSM.Apply` decodes the committed `internal/raft/proto.Entry` and
   checks `entry.NodeID == f.NodeID()`. If it matches (this node proposed it),
   **return without touching the WAL at all** — this node's own SQLite write
   path already published it (`§write path` step 4). Otherwise, continue.

   **This check is a known, tracked correctness bug in its current form —
   see `DECISIONS.md` ADR-011 before relying on it for anything beyond the
   straightforward case.** It's a permanent, static check on `NodeID`, not
   scoped to the one proposal that's actually safe to skip, and it silently
   drops entries in at least two reproducible scenarios (hraft's Figure-8
   retroactive commit; an ordinary restart after this node has taken its own
   RAFT snapshot). Both are demonstrated by `PIt`/pending specs
...
`Restore`, on the installing follower, is **not** a whole-file swap the way
the pre-refactor `dbBackend.Restore` was (that version closed every open
connection, atomically renamed the incoming bytes in as the new `.db`, and
reopened everything fresh). The current version instead treats the incoming
snapshot bytes as a sequence of whole database pages — page 1, page 2, ...,
page N, read directly off the byte stream at fixed `pageSize` strides — and
appends them as one `internal/fsm/walappender` entry (frame `i` is page
`i+1`, `nTruncate` set to the page count on the last frame), then runs a
`TRUNCATE` checkpoint. This reuses the exact same append/publish machinery
`§follower-apply` steps 2–6 already need, rather than needing a second,
separate "swap the whole file and reopen every connection" code path. It
validates the incoming bytes are a whole multiple of the page size and that
the snapshot's own page-size field (read from its own page 1) matches this
node's configured page size, rejecting otherwise. No connection ever needs
to close and reopen for this to work — `fsm.FSM`'s own long-lived connection
and its `dbLock` (`§external-reader safety` below) stay untouched throughout.
...
disconnecting. Real SQLite's own `sqlite3_close` path is, correctly, willing
...
from under the node, silently orphaning every not-yet-checkpointed
...
- *Gaining leadership:* a node can be **apply-behind** (has every committed log
  ...
  it *can* win an election, but its SQLite state is stale. `internal/raft/gate.Gate`
  tracks a `ready` flag, closed (`false`) the instant a leadership term begins
  and opened only once a current-term `hraft.Barrier` call returns — which by
  construction blocks until every already-committed entry, including any
  backlog, has been sent through `fsm.FSM.Apply` on this node (`Gate.drain`).
  `Gate.Propose` rejects with `CatchingUpError` while closed.

  This drain is also what the pre-refactor design relied on to close the
  Figure-8 self-apply race: the stale entry from an earlier, unfinished
  leadership stint is always applied *during* the drain, before any new
  self-proposal races to claim the (then-transient) self-apply marker. The
  current `entry.NodeID == f.NodeID()` check has no such transient state to
  race over — which is exactly why it fails this case differently (silently
  skipping the entry forever, rather than racing to skip it once) rather
  than being made safe by the drain. **See `DECISIONS.md` ADR-011.**
...
for it yet, despite an earlier draft of this document implying one existed.
...
- You can skip/batch the local WAL fsync. A crashed node rebuilds its WAL tail
  from the RAFT log on restart, via hraft's own snapshot-restore + log-replay
  (`vendor/github.com/hashicorp/raft`'s `restoreSnapshot`/`processLogs`),
  idempotent by construction since RAFT entries are full page images, not
  deltas — replaying an already-applied entry converges to the same state
  rather than corrupting it. (This idempotence does *not* currently cover the
  self-apply skip's own known bug — see ADR-011 — which is a distinct failure
  mode from ordinary crash/restart recovery.)
