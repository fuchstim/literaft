...
re-verify against whatever's pinned there, this drifts). Original notes were
verified against v0.35.1; nothing in this file changed between those two
versions as far as this project's usage goes, but the internals below are
...
kernel, so coordination works — verified directly, not just assumed:
`internal/vfs/external_reader_test.go` drives the real system `sqlite3` CLI as
an external reader against a live wrapper-VFS writer.
...
compatibility class (`unix-flock`). Use the **default** build (CLAUDE.md
already says this; repeated here because it's the reason the rest of this
file's claims hold).
...
- **Optional File capability interfaces** — a `File` may also implement
  ...
  so shims can expose the wrapped file — mirror it. `internal/vfs/file.go`'s
  long list of `var _ sqlite3vfs.FileXxx = (*File)(nil)` assertions plus
  `vfsutil.WrapXxx` forwarding calls is this requirement, made exhaustive.
...
one to wrap via `vfs.Find("")` (or `vfs.Find("os")`, also reserved for the
...
## The *other* lock: the main `.db` file's own SHARED lock (not in `-shm` at all)
...
This project didn't need to know about this second lock until
`DECISIONS.md` ADR-012: a node's own long-lived connections don't reliably
hold it just by being open and otherwise idle, which meant an ordinary,
transient external reader (requirement #3) could legitimately win that
upgrade race and delete a node's `-wal` out from under it. `fsm/dblock.go`
now takes this lock explicitly and holds it for the node's whole lifetime —
see that ADR and `WAL_FORMAT.md` for the mechanics.
...
- Keep at least one RW connection open per process so the wal-index stays
  live for external readers — and, since `DECISIONS.md` ADR-012, hold the
  main-`.db`-file shared lock explicitly too; a kept-alive connection alone
  turned out not to be sufficient for the latter.
