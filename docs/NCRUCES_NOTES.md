...
Re-verify against whatever is pinned there: the internals below are
...
- There is **no C `unix` VFS to delegate to.** "Base VFS" = ncruces' own default
  Go VFS. The wrapper (`internal/vfs`) wraps *that*.
- On Linux/macOS the default VFS uses **`mmap`** for the wal-index shared memory
  and **OFD locks** for file locking: the same mechanisms SQLite uses. This is
...
Requirement #3 (an *unmodified* stock SQLite process opens the db read-only,
concurrently) rests on lock interoperability:
...
kernel, so coordination works. This is verified directly by
...
cross-platform, **in-process** sharing implementation: WAL dbs can then only
...
## The exported VFS surface

From `github.com/ncruces/go-sqlite3/vfs` (names as of v0.35.2):

- `Register(name, vfs)` / `Unregister(name)` / `Find(name)`: registers the
  wrapper under a name passed via `?vfs=` in the DSN (`internal/vfs.Register`
  does this).
...
- **Optional File capability interfaces**: a `File` may also implement
  ...
  these must be forwarded through (type-assert the underlying file and
  re-expose); otherwise WAL/shm and other features are silently disabled.
  `FileUnwrap` exists so shims can expose the wrapped file; it must be
  mirrored. `internal/vfs/file.go`'s long list of
  `var _ sqlite3vfs.FileXxx = (*File)(nil)` assertions plus `vfsutil.WrapXxx`
  forwarding calls implements this requirement exhaustively.
- `Filename`: has `Database()`, `Journal()`, `WAL()`, `URIParameter(...)`. Used
  ...
  journal) so writes are intercepted only on the WAL.
...
## Why a custom shm layer is needed
...
returns a value satisfying that interface, but it can only be handed *back* to
the library: its methods cannot be called, nor can the interface be
implemented from outside the package.
...
shm code. It must stay in lockstep with the on-disk wal-index layout and
...
output, gitignored, a full mirror of every dependency), which is refreshed
with a plain `go mod vendor` when needed and never hand-edited.

**Coordination within one process.** The follower-apply shm handle and
SQLite's own shm handle both map the same `-shm` file (`MAP_SHARED`) and use
OFD locks. Two FDs in the same process *do* contend on OFD locks (per
open-file-description, not per-process), and two mmaps of the same file are
coherent. The follower-apply handle therefore coordinates with SQLite's
handle exactly as two processes would.

---

## The *other* lock: the main `.db` file's own SHARED lock

Distinct from the locks above: every connection with a database open, WAL
mode included, also holds a plain OS-level **SHARED** lock on the *main*
`.db` file itself (not `-shm`), for as long as it's open, using the exact
same classic byte-range locking scheme non-WAL SQLite always used
(`os_unix.c`'s `SHARED_FIRST`/`SHARED_SIZE`, see `WAL_FORMAT.md §main .db
file locking`). This lock does nothing to coordinate readers and writers
during normal WAL operation: that role belongs to the wal-index's own
read-mark slots and `WAL_WRITE_LOCK` (previous section). Its only real
purpose is `sqlite3_close`'s "am I the last connection with this database
open anywhere?" check: closing tries to upgrade this shared lock to
exclusive, which only succeeds if no other connection, in this or any other
process, still holds it. If it succeeds, that connection is entitled to
...
## wal-index page-map ≠ the future OCC pagemap

Stated in CLAUDE.md; noted here because `internal/fsm/walappender/shm/`
...
- `?vfs=<registered-name>` in the DSN selects the wrapper (`driver.Driver`'s
  `dsn()` builds this automatically; a caller driving
...
- `nolock=1`/`EXCLUSIVE` locking (unsupported-platform fallbacks) is
  **incompatible with requirement #3** and with the `database/sql` pool
  (`db.SetMaxOpenConns(1)` required); avoid on the main path.
