...
var _ = Describe("forwardinggate.Gate", func() {
	...
	It("rejects a forwarded write whose base index is stale, without proposing it", func() {
		...
		Expect(c.LeaderGate(leader).ProposeTransaction(primer.frames)).To(Succeed())
		...
		req := &raftproto.ForwardRequest{
			Header: &raftproto.ForwardRequest_Header{LastAppliedIndex: 0},
			Entry:  entry,
		}

		resp, err := c.hub.Transport(follower.Addr).Propose(context.Background(), leader.Addr, req)
		Expect(err).NotTo(HaveOccurred())
		Expect(resp.GetStatus()).To(Equal(raftproto.ForwardResponse_STATUS_STALE_BASE))
	...
	It("rejects a malformed transaction shape without proposing it", func() {
		...
		resp, err := c.hub.Transport(follower.Addr).Propose(context.Background(), leader.Addr, req)
		Expect(err).NotTo(HaveOccurred())
		Expect(resp.GetStatus()).To(Equal(raftproto.ForwardResponse_STATUS_INVALID))
