...
// VFS implements both (vendor/github.com/ncruces/go-sqlite3/vfs/vfs.go's
// vfsOpen), so every other test in this package -- all opened via
// sqlite3.Open's URI DSNs -- exercises VFS.OpenFilename, never VFS.Open
// itself; a real SQLite connection can't reach it once OpenFilename exists.
// This calls Open directly to cover the plain path Wrap/WrapGate construct.
// The base vfsOS.Open only ever supports an empty name (temp files) and
// returns _CANTOPEN for anything else (its own doc: "OpenFilename is called
// instead"), so that's the only call shape worth exercising here.
var _ = Describe("VFS.Open (the non-Filename path)", func() {
	It("wraps a directly-opened temp file, and the wrapped file behaves like a plain file", func() {
		v := raftvfs.Wrap(sqlite3vfs.Find(""))
