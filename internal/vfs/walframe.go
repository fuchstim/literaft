...
const (
	...
	// Every frame starts at walHeaderSize + n*(frameHeaderSize+page size),
	// which is what lets WriteAt tell a frame header write from a
	// page-image write by offset alone, without depending on which write
	// came before it.
	walHeaderSize = 32

	// frameHeaderSize is the fixed size of a WAL frame header, in bytes.
	// SQLite's minimum page size (512) rules out any ambiguity between a
	// frame header write, the 32-byte WAL header, and a frame's page-image
	// write.
	frameHeaderSize = 24
)
...
// header. All WAL fields are big-endian.
