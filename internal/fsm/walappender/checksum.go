...
// over the input, taken 8 bytes (two u32 words) at a time.
//
...
// regardless of host CPU, so this engine's "native" checksum order is
// always little-endian and its WAL magic low bit (bigEndCksum) is always
// 0. len(b) must be a positive multiple of 8.
