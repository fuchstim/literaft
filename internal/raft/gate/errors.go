...
// leader. A follower rejects the write
...
// an election but hasn't yet finished draining its apply backlog. The node is the
// raft leader but its local SQLite state may not yet reflect every entry the
